...
//     */
//    private void reInitializeBufferOrig(ByteBuffer buf, BitSet bitInfo, int recordNumber,
...
System.out.println("setStartingRecordNumber: set to " + recordNumber);
     ...
     * common record which is stored in the user-header part of the file header.
     * <p>
     *
     ...
     * the buffer is never split. Writing this event is done by writing the
     * common record as the first data record if it has not been already done.
     * If the common record was already written, then the event is written as
     * any other.<p>
     *
     * Do not call this while simultaneously calling
    ...
    public void setFirstEvent(EvioNode node) throws EvioException, IOException {
        ...
        if (toFile) {
            ...
            if (recordsWritten < 1) {
                // The new common record will be written to file header, later
                return;
            }
        }
        else {
            // Common record is written in constructor, so if no common record
            // is written yet, then there is none. Create it and write it to buf.
            if (recordsWritten < 1) {
                createCommonRecord(xmlDictionary, null, node, null);
                flushCommonRecordToBuffer();
                return;
        ...
        // Common record is already written, so write first event like all others
...
System.out.println("Buffer size (" + bufferSize + ") - bytesWritten (" + bytesWrittenToBuffer +
      ") - trailer (" + trailerBytes() +  ") = (" +
         ((bufferSize - bytesWrittenToBuffer) >= bytes + RecordHeader.HEADER_SIZE_BYTES) +
      ") >= ? " + bytes);
        return toFile() || ((bufferSize - bytesWrittenToBuffer - trailerBytes()) >= bytes);
    }
...
//System.out.println("    writeToFile: IN");
    ...
    private void writeTrailerToFile(boolean writeIndex) throws IOException {
        ...
        if (!writeIndex) {
            try {
                ...
                FileHeader.writeTrailer(headerBuffer, 0, recordNumber, byteOrder, null);
            }
            catch (HipoException e) {/* never happen */}
            ...
            try {
                FileHeader.writeTrailer(headerBuffer, 0, recordNumber,
...
//    private void flushCurrentRecordToBufferOrig() {
//
...
//
//        // Do compression
////System.out.println("   EventWriterUnsync: flushCurrentRecordToBuffer, build record");
//        currentRecord.build();
//
//        int bytesToWrite = header.getLength();
...
//        recordLengths.add(eventCount);
//
////System.out.println("   EventWriterUnsync: header = \n" + header);
////
////        System.out.println("   EventWriterUnsync: dest buffer pos = " + buffer.position() +
////                           ", cap = " + buffer.capacity() + ", lim = " + buffer.limit());
////        System.out.println("                    : src buffer cap = " + buffer.capacity() +
...
//        else {
//            buffer.put(buf);
//// TODO: THIS WILL FAIL, since position is wrong!! and pos will changed too!!!
//        }
//
////Utilities.printBuffer(buf, 0, 33, "EventWriterUnsync, internal buffer --->");
...
//        eventsWrittenTotal    += eventCount;
//        buffer.position(bytesWrittenToBuffer);
//
////System.out.println("flushCurrentRecordToBuffer: set record # to " + header.getRecordNumber() +
////                   ", increase to " + recordNumber);
//
//
////System.out.println("   EventWriterUnsync: bytesWritten = " + bytesWritten + ", byesToWrite = " + bytesToWrite +
////", record # just written = " + (recordNumber -1));
////System.out.println("flushCurrentRecordToBuffer: buf pos, lim = " + buffer.position() +
////                   ", " + buffer.limit());
//
//        currentRecord.reset();
//
////System.out.println("flushCurrentRecordToBuffer: reset current record, set record # to " + recordNumber);
//    }
...
//     * @throws EvioException if this object already closed; if user-given buffer is
//     *                       too small to fit single event.
//     */
//    private void writeToBufferOrig(EvioBank bank, ByteBuffer bankBuffer, boolean force)
//                                    throws EvioException {
//        if (closed) {
//            throw new EvioException("close() has already been called");
//        }
...
//    }
     ...
     * A bank in buffer form has priority,
     * if it's null, then it looks at the bank in EvioBank object form.
     *
     * @param bank        bank to write in EvioBank object form.
     * @param bankBuffer  bank to write in buffer form .
