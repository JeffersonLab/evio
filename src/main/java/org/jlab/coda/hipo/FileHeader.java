...
public class FileHeader implements Cloneable {
    ...
    /**
     * 9th bit set in bitInfo word in file header means every split file has same
     * first event.
     */
    final static int FIRST_EVENT_BIT = 0x200;
    /**
     * 10th bit set in bitInfo word in file header means file trailer with index
     * array exists.
     */
    ...
    private static int getWords(int length) {
        int words = length / 4;
        if (getPadding(length) > 0)
            words++;
        return words;
    }

    /**
     * Returns number of bytes needed to pad to 4-byte boundary for the given
     * length.
     * 
     * @param length length in bytes.
    ...
    /**
     * Get the total length of header + index + user header (including padding) in
     * bytes.
     * Never compressed.
     * 
     * @return total length of header + index + user header (including padding) in
     *         bytes.
     */
    ...
    /**
     * Get the user header's padding - the number of bytes required to bring
     * uncompressed
    ...
    public int getUserHeaderLengthPadding() {
        return userHeaderLengthPadding;
    }
     ...
     * @param haveTrailerWithIndex true if file has trailer with record length
     *                             index.
    ...
    /**
     * Does this bitInfo arg indicate the existence of a first event in the file
     * header?
     * 
     * @param bitInfo bitInfo word.
     * @return true if header has a first event in the file header, else false.
     */
    static public boolean hasFirstEvent(int bitInfo) {
        return ((bitInfo & FIRST_EVENT_BIT) != 0);
    }
    ...
    /**
     * Does this bitInfo arg indicate the existence of a dictionary in the file
     * header?
     * 
     * @param bitInfo bitInfo word.
     * @return true if header has a dictionary in the file header, else false.
     */
    static public boolean hasDictionary(int bitInfo) {
        return ((bitInfo & DICTIONARY_BIT) != 0);
    }

    /**
     * Set the bit in the file header which says there is a trailer with a record
     * length index.
     * 
     * @param hasTrailerWithIndex true if file has a trailer with a record length
     *                            index.
     * @return new bitInfo word.
     */
    ...
    /**
     * Does this bitInfo arg indicate the existence of a trailer with a record
     * length index?
     * 
     * @param bitInfo bitInfo word.
     * @return true if file has a trailer with a record length index, else false.
    ...
    public boolean hasUserHeader() {
        return userHeaderLength > 0;
    }

    /**
     * Does this file have a valid index of record lengths immediately following
     * header?
    ...
    public boolean hasIndex() {
        return ((indexLength > 3) && (indexLength % 4 == 0));
    }
    ...
    /**
     * Set the user header's padding - the number of bytes required to bring
     * uncompressed
    ...
    private void setUserHeaderLengthPadding(int padding) {
        // System.out.println("setUserHeaderLengthPadding: IN, firstEv bit = " +
        // hasFirstEvent());
        userHeaderLengthPadding = padding;
        bitInfo = bitInfo | (userHeaderLengthPadding << 20);
        // System.out.println("setUserHeaderLengthPadding: END, firstEv bit = " +
        // hasFirstEvent());
    }
    ...
    public FileHeader setHeaderLength(int length) {
        if (length < 56) {
            length = 56;
            // System.out.println("setHeaderLength: attempting to set header length too
            // small (" + hasFirstEvent() + ")");
        }
        headerLength = length;
    ...
    /**
     * Writes the file (not record!) header into the given byte buffer starting at
     * beginning.
     * 
     ...
     *                       data is not in proper format, or version earlier than
     *                       6.
     */
     ...
     * @throws HipoException if buffer is not in the proper format or earlier than
     *                       version 6
     */
    ...
    /**
     * Run this class as an executable which tests the writing and reading of a
     * record.
     * 
     * @param args args
     */
