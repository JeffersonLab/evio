...
namespace evio {
     ...
     * @since 4.0
     */
    LinkedHashMap<EvioDictionaryEntry, std::string> tagNumMap = new LinkedHashMap<>(100);
     ...
     * @since 4.1
     */
    LinkedHashMap<EvioDictionaryEntry,std::string> tagOnlyMap = new LinkedHashMap<>(20);
     ...
     * @since 5.2
     */
    LinkedHashMap<EvioDictionaryEntry,std::string> tagRangeMap = new LinkedHashMap<>(20);
     ...
     * @since 5.2
     */
    LinkedHashMap<std::string,EvioDictionaryEntry> reverseMap = new LinkedHashMap<>(100);
     ...
     * @since 4.0
     */
    LinkedHashMap<std::string,EvioDictionaryEntry> tagNumReverseMap = new LinkedHashMap<>(100);
...
public:
     ...
     */
    EvioXMLDictionary(File file, std::string delimiter) {
     ...
     */
    EvioXMLDictionary(std::string xmlString) {
     ...
     * @param delimiter character used to separate hierarchical parts of names.
     */
    EvioXMLDictionary(std::string xmlString, std::string delimiter) {
     ...
     */
    EvioXMLDictionary(Document domDocument) {
         ...
         * @param delimiter character used to separate hierarchical parts of names.
         */
        EvioXMLDictionary(Document domDocument, std::string delimiter) {
...
// Look at all the children, if any
            ...
            std::string name, tagStr, tagEndStr, numStr, numEndStr, type, format, description;
...
// recursively, and add all existing entries.
     ...
     *          containing its data (tag, num, type, etc.).
     */
    Map<std::string, EvioDictionaryEntry> getMap() {
    ...
    private:
         ...
         */
        void addHierarchicalDictEntries(NodeList kidList, std::string parentName) {

            if (kidList == null || kidList.getLength() < 1) return;


            Integer tag, tagEnd, num, numEnd;
            bool isLeaf, badEntry, isTagRange, isNumRange;
            std::string name, tagStr, tagEndStr, numStr, numEndStr, type, format, description;
...
//System.out.println("FOUND FORMAT: = " + format);
     ...
     */
     std::string getName(BaseStructure structure) {
     ...
     */
    std::string getName(Integer tag, Integer num) {
     ...
     * @return descriptive name or "???" if none found
     */
    std::string getName(Integer tag,  Integer num,  Integer tagEnd,
     ...
     */
    std::string getName(EvioDictionaryEntry key) {
        int tag = key.getTag();
        ...
        switch (entryType) {
            ...
            case TAG_RANGE:
                ...
                if (name != null) {
                    ...
                    for (Map.Entry<EvioDictionaryEntry, std::string> item : tagRangeMap.entrySet()) {
    ...
    //                                   ", tagEnd = " + key.getTagEnd() + ", num = " + key.getNum());
         ...
         */
        EvioDictionaryEntry entryLookupByData(Integer tag, Integer num, Integer tagEnd) {
            ...
            switch (entryType) {
                ...
                case TAG_ONLY:
                    ...
                    if (name != null) {
                        ...
                        for (Map.Entry<EvioDictionaryEntry, std::string> item : tagOnlyMap.entrySet()) {
                            std::string n = item.getValue();
                            if (n.equals(name)) {
                                entry = item.getKey();
                                break outer;
                            }
         ...
         * @return entry or null if none found
         */
        EvioDictionaryEntry entryLookupByName(std::string name) {
            // Check all entries
 ...
 */
        std::string getDescription(Integer tag, Integer num) {
 ...
 * @return description or null if none found
 */
        std::string getDescription(Integer tag, Integer num, Integer tagEnd) {
 ...
 */
        std::string getFormat(int tag, int num) {
 ...
 */
        std::string getFormat(Integer tag, Integer num, Integer tagEnd) {
            EvioDictionaryEntry entry = entryLookupByData(tag, num, tagEnd);
            if (entry == null) {
                return null;
            }
 ...
 */
        DataType getType(Integer tag, Integer num) {
 ...
 * @return type or null if none found
 */
        DataType getType(Integer tag, Integer num, Integer tagEnd) {
            EvioDictionaryEntry entry = entryLookupByData(tag, num, tagEnd);
            if (entry == null) {
                return null;
            }
 ...
 */
        Integer[] getTagNum(std::string name) {
            EvioDictionaryEntry entry = entryLookupByName(name);
            if (entry != null) {
 ...
 */
        Integer getTag(std::string name) {
            EvioDictionaryEntry entry = entryLookupByName(name);
            if (entry == null) {
                return null;
            }
 ...
 */
        Integer getTagEnd(std::string name) {
            EvioDictionaryEntry entry = entryLookupByName(name);
            if (entry == null) {
                return null;
            }
         ...
         */
        Integer getNum(std::string name) {
            EvioDictionaryEntry entry = entryLookupByName(name);
            if (entry == null) {
                return null;
            }
    ...
    private:
         ...
         */
        static Document getDomObject(File file) {
// get the factory
...
// parse using builder to get DOM representation of the XML file
         ...
         * @return the dom object (or <code>null</code>.)
         */
        static Document getDomObject(std::string xmlString) {
// get the factory
            DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
            Document dom = null;
 ...
 */
        std::string toString() {
            ...
            for (std::string name : reverseMap.keySet()) {
                // Get the entry
                ...
                switch (entry.getEntryType()) {
                    case TAG_RANGE:
                        sb.append(std::string.format("%-30s: tag range %d-%d\n", name, tag, tagEnd));
                        break;
                    case TAG_ONLY:
                        sb.append(std::string.format("%-30s: tag %d\n", name, tag));
                        break;
                    case TAG_NUM:
                        sb.append(std::string.format("%-30s: tag %d, num %d\n", name, tag, num));
                        break;
                    default:
                }
