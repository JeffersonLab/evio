...
//    static int  swapComposite_t(uint32_t *data, int tolocal, uint32_t *dest, uint32_t length);
     ...
     */
    static void swapData(uint32_t *data, int type, uint32_t length, bool toLocal, uint32_t *dest) {
        ...
        switch (type) {
            ...
            case 0x0:
                ByteOrder::byteNoSwap32(data, length, dest);
                break;
            ...
            case 0x2:
            case 0xb:
                ByteOrder::byteSwap32(data, length, dest);
                break;
            ...
            case 0x4:
            case 0x5:
                ByteOrder::byteSwap16(reinterpret_cast<uint16_t *>(data), 2*length,
            ...
            case 0x9:
            case 0xa:
                ByteOrder::byteSwap64(reinterpret_cast<uint64_t *>(data), length/2,
            ...
            case 0x10:
                while (l < length) {
                    ...
                    if (toLocal) {
                        // swap bank
                        swapBank(&data[l], toLocal, (dest==nullptr) ? nullptr : &dest[l]);
                        // bank was this long (32 bit words) including header
                        fraglen = (dest==nullptr) ? data[l]+1: dest[l]+1;
                    }
                    else {
                        fraglen = data[l] + 1;
                        swapBank(&data[l], toLocal, (dest==nullptr) ? nullptr : &dest[l]);
            ...
            case 0x20:
                while (l < length) {
                    if (toLocal) {
                        swapSegment(&data[l], toLocal, (dest==nullptr) ? nullptr : &dest[l]);
                        fraglen = (dest==nullptr) ? (data[l] & 0xffff) + 1 : (dest[l] & 0xffff) + 1;
                    }
                    else {
            ...
            case 0xc:
                while (l < length) {
                    if (toLocal) {
                        swapTagsegment(&data[l], toLocal, (dest==nullptr) ? nullptr : &dest[l]);
                        fraglen = (dest==nullptr) ? (data[l] & 0xffff) + 1 : (dest[l] & 0xffff) + 1;
                    } else {
                        fraglen = (data[l] & 0xffff) + 1;
                        swapTagsegment(&data[l], toLocal, (dest==nullptr) ? nullptr : &dest[l]);
