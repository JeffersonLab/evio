...
public:
  static uint32_t *appendToBuffer(const uint32_t *buffer, ContainerType bufferType,
...
public:
  ...
  virtual void setDictionary(const evioDictionary &dict)  {toStringDictionary=&dict;}
  virtual const evioDictionary *getDictionary() const {return(toStringDictionary);}
...
public:
  ...
  template <typename T> void addBank(evioDictEntry tn, const vector<T> &dataVec);
  template <typename T> void addBank(evioDictEntry tn, const T* dataBuf, int dataLen);
...
public:
  ...
  template <typename T> vector<T> *getVectorUnique();
