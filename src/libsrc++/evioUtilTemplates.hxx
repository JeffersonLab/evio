...
template <> class evioUtil<uint32_t>     {public: static int evioContentType()   {return(0x1);}};
template <> class evioUtil<float>        {public: static int evioContentType()   {return(0x2);}};
template <> class evioUtil<string>       {public: static int evioContentType()   {return(0x3);}};
template <> class evioUtil<string&>      {public: static int evioContentType()   {return(0x3);}};
template <> class evioUtil<int16_t>      {public: static int evioContentType()   {return(0x4);}};
template <> class evioUtil<uint16_t>     {public: static int evioContentType()   {return(0x5);}};
template <> class evioUtil<int8_t>       {public: static int evioContentType()   {return(0x6);}};
template <> class evioUtil<uint8_t>      {public: static int evioContentType()   {return(0x7);}};
template <> class evioUtil<double>       {public: static int evioContentType()   {return(0x8);}};
template <> class evioUtil<int64_t>      {public: static int evioContentType()   {return(0x9);}};
template <> class evioUtil<uint64_t>     {public: static int evioContentType()   {return(0xa);}};
template <> class evioUtil<int32_t>      {public: static int evioContentType()   {return(0xb);}};
...
template <typename T> evioDOMNodeP evioDOMNode::createEvioDOMNode(uint16_t tag, uint8_t num) {
  return(new evioDOMLeafNode<T>(nullptr,tag,num));
}
...
template <typename T> evioDOMNodeP evioDOMNode::createEvioDOMNode(const string &name, const evioDictionary *dictionary)  {
  ...
  if(dictionary!=nullptr) {
    evioDictEntry entry = dictionary->getEntry(name);
    return(new evioDOMLeafNode<T>(nullptr,entry.getTag(),entry.getNum()));
  } else {
...
template <typename T> evioDOMNodeP evioDOMNode::createEvioDOMNode(uint16_t tag, uint8_t num, const vector<T> &tVec) {
  return(new evioDOMLeafNode<T>(nullptr,tag,num,tVec));
}
 ...
 * @return Pointer to new node
 */
template <typename T> evioDOMNodeP evioDOMNode::createEvioDOMNode(const string &name,
        const evioDictionary *dictionary, const vector<T> &tVec) {

  if(dictionary!=nullptr) {
    evioDictEntry entry = dictionary->getEntry(name);
    return(new evioDOMLeafNode<T>(nullptr,entry.getTag(),entry.getNum(),tVec));
  } else {
    throw(evioException(0,"?evioDOMNode constructor...NULL dictionary for bank name: " + name,__FILE__,__FUNCTION__,__LINE__));
  }
...
template <typename T> evioDOMNodeP evioDOMNode::createEvioDOMNode(uint16_t tag, uint8_t num, const T* t, int len)
   {
  return(new evioDOMLeafNode<T>(nullptr,tag,num,t,len));
}
...
template <typename T> evioDOMNodeP evioDOMNode::createEvioDOMNode(const string &name, const evioDictionary *dictionary, const T* t, int len)
  ...
  if(dictionary!=nullptr) {
    evioDictEntry entry = dictionary->getEntry(name);
    return(new evioDOMLeafNode<T>(nullptr,entry.getTag(),entry.getNum(),t,len));
  } else {
    throw(evioException(0,"?evioDOMNode constructor...NULL dictionary for bank name: " + name,__FILE__,__FUNCTION__,__LINE__));
  }
...
template <typename T> evioDOMNodeP evioDOMNode::createEvioDOMNode(const string &name, const evioDictionary *dictionary, T *t, 
  ...
  if(dictionary!=nullptr) {
    evioDictEntry entry = dictionary->getEntry(name);
    evioDOMContainerNode *c = new evioDOMContainerNode(nullptr,entry.getTag(),entry.getNum(),cType);
 ...
 */
template <typename T> vector<T> *evioDOMNode::getVector()  {
  if(contentType!=evioUtil<T>::evioContentType())return(nullptr);
  evioDOMLeafNode<T> *l = static_cast<evioDOMLeafNode<T>*>(this);
  return(&l->data);
}
...
template <class Predicate> evioDOMNodeListP evioDOMNode::getChildren(Predicate pred)  {
  ...
  //  return(std::move(l)); // timmer: change this
  return(l);
}
 ...
 * Destructor
 */
template <typename T> evioDOMLeafNode<T>::~evioDOMLeafNode() {
}
...
template <typename T> string evioDOMLeafNode<T>::getHeader(int depth, const evioToStringConfig *config) const {
  ...
  if(name.size()<=0) name = evGetTypename(parent==nullptr?BANK:parent->getContentType());
 ...
 */
template <typename T> int evioDOMLeafNode<T>::getSize() const {
  return(data.size());
}
...
template <class Predicate> evioDOMNodeP evioDOMTree::findFirstNode(evioDOMNodeP pNode, Predicate pred)  {
  ...
  if(pNode->isLeaf())return(nullptr);
 ...
 * @return Pointer to vector, nullptr if no node contains vector<T>
 */
template <typename T> vector<T> *evioDOMTree::getVectorUnique()  {
 ...
 * @return Pointer to vector<T>, nullptr if no node containing vector<T> satisfies predicate
 */
...
/**
 * Creates leaf node and adds it to tree root node.
 * @param entry Leaf node evioDictEntry
 * @param dataVec vector<T> of data
 */
template <typename T> void evioDOMTree::addBank(evioDictEntry entry, const vector<T> &dataVec)  {
  addBank(entry.getTag(),entry.getNum(),dataVec);
  return;
}
 ...
 * Creates leaf node and adds it to tree root node.
 * @param entry Leaf
 * node evioDictEntry @param dataBuf Pointer to array containg data of type T
 * @param dataLen Length of array
 */
template <typename T> void evioDOMTree::addBank(evioDictEntry entry, const T* dataBuf, int dataLen)  {
  addBank(entry.getTag(),entry.getNum(),dataBuf,dataLen);
  return;
}
...
public:
  typeIs() : type(evioUtil<T>::evioContentType()) {}
...
public:
  ...
  tagNumEquals(evioDictEntry entry) : tag(entry.getTag()), num(entry.getNum()) {}
  ...
  tagNumEquals(const string &name, const evioDOMTree *tree) : tag(0), num(0) {
    if((tree!=nullptr)&&(tree->dictionary!=nullptr)) {
      evioDictEntry entry = tree->dictionary->getEntry(name);
      tag=entry.getTag();
      num=entry.getNum();
    } else {
...
public:
  ...
  bool operator()(const evioDOMNodeP node) const {return((node->getParent()==nullptr)?false:(node->getParent()->getContentType()==type));}
private:
  int type;
};
...
public:
  ...
  bool operator()(const evioDOMNodeP node) const {return((node->getParent()==nullptr)?false:(node->getParent()->tag==tag));}
private:
  uint16_t tag;
};
...
public:
  ...
  bool operator()(const evioDOMNodeP node) const {return((node->getParent()==nullptr)?false:(node->getParent()->num==num));}
private:
  uint8_t num;
};
...
public:
  ...
  parentTagNumEquals(evioDictEntry entry) : tag(entry.getTag()), num(entry.getNum()) {}
  bool operator()(const evioDOMNodeP node) const {
    return((node->getParent()==nullptr)?false:((node->getParent()->tag==tag)&&(node->getParent()->num==num)));}
private:
  uint16_t tag;
  uint8_t num;
};
