...
static void swap_short(uint16_t *data, uint32_t length, uint16_t *dest);
...
// TODO: should tolocal play a role here ?
            ...
            /* copy if needed */
            if (dest != NULL) {
                copy_data(&(data[1]), formatLen, &(d[1]));
            }
