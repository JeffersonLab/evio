...
public class EventWriterV4 {
    ...
    /**
     * Creates an <code>EventWriterV4</code> for writing to a file in native byte order.
     * If the file already exists, its contents will be overwritten unless
     * it is being appended to. If it doesn't exist, it will be created.
     *
     * @param file       the file object to write to.<br>
    ...
    /**
     * Create an <code>EventWriterV4</code> for writing events to a file.
     * If the file already exists, its contents will be overwritten.
     * If it doesn't exist, it will be created.
     *
     * @param file          the file object to write to.<br>
    ...
    public EventWriterV4(String baseName, String directory, String runType,
        ...
        if (blockSizeMax > EventWriterV4.MAX_BLOCK_SIZE) {
            throw new EvioException("blockSizeMax arg must be smaller");
        }

        if (blockCountMax < EventWriterV4.MIN_BLOCK_COUNT) {
            throw new EvioException("blockCountMax arg must be bigger");
        }

        if (blockCountMax > EventWriterV4.MAX_BLOCK_COUNT) {
        ...
        try {
            if (append) {
                ...
                if (asyncFileChannel.size() >= 32) {
                ...
                else if (asyncFileChannel.size() > 0) {
                    ...
                    // File size is 0, so we are not appending
        ...
        if (xmlDictionary == null) {
            writeNewHeader(0,blockNumber++,bitInfo,
                    false,false, haveFirstEvent);
        }
        else {
            writeNewHeader(0,blockNumber++,bitInfo,
                    true,false, haveFirstEvent);
        }
    ...
    private void initializeBuffer(ByteBuffer buf, int blockSizeMax, int blockCountMax,
        ...
        if (xmlDictionary == null) {
            writeNewHeader(0,this.blockNumber++,bitInfo,
                    false,false, haveFirstEvent);
        }
        else {
            writeNewHeader(0,this.blockNumber++,bitInfo,
    ...
    public void setBuffer(ByteBuffer buf, BitSet bitInfo, int blockNumber) throws EvioException {
        ...
        if (!closed) {
            throw new EvioException("Close EventWriterV4 before changing buffers");
        }
    ...
    synchronized public void close() {
        ...
        try {
            if (toFile) {
                ...
                // Add that now.
                writeNewHeader(0, blockNumber, null, false, true, false);
...
//if (debug) System.out.println("      resetBuffer: as in constructor");
                ...
                writeNewHeader(0, blockNumber++, null,
                        (xmlDictionary != null), false, haveFirstEvent);
            }
            else {
...
//                bytesWrittenToBuffer);
        ...
        buffer.putInt(currentHeaderPosition, currentBlockSize);
        buffer.putInt(currentHeaderPosition + EventWriterV4.EVENT_COUNT_OFFSET,
                      currentBlockEventCount);
...
//                currentBlockEventCount + ", blk # = " + blockNumber + ", wrote Dict = " + wroteDictionary);
        ...
        int headerInfoWord = buffer.getInt(currentHeaderPosition + EventWriterV4.BIT_INFO_OFFSET);
        if (BlockHeaderV4.isLastBlock(headerInfoWord)) {
//System.out.println("  writeEventToBuffer: clear last event bit in block header");
            buffer.putInt(currentHeaderPosition + EventWriterV4.BIT_INFO_OFFSET,
