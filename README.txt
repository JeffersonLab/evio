...
scons tar             create a tar file (evio-5.x.tgz) of the evio top level
...
One can download the jevio-5.2.jar file from the CODA website,
it can be downloaded from the github site in the java/jars/java8 or java15
directories, or it can be generated from the general evio distribution.
Note that the jar files in java/jars/java8 have been compiled with Java version 8
while thos in java15 have been compiled with Java version 15.
In any case, put the jar file into your classpath and run your java application.

If you're using the jar file from the CODA website, java version 8 or
higher is necessary since it was compiled with version 8. If
generating it, use java version 8 or higher to compile.
...
However, if using the distribution directly, some of the documentation
